...
def saham(kode_saham):
    saham_relevan = Saham.query.filter_by(kode=kode_saham.upper()).one()
    berita_relevan = Berita.query.filter_by(kode_saham=kode_saham.upper()).order_by(
        Berita.tanggal_publish.desc()).limit(5)
    rand_saham = Saham.query.order_by(db.func.random()).first()

    today = datetime.datetime.now().strftime('%d-%m-%Y')
    df = web.DataReader(
        kode_saham.upper()+".JK", data_source='yahoo', start='01-01-2022', end=today)
    df = df.filter(['Close'])
